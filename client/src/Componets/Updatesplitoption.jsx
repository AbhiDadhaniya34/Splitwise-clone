...
function Updatesplitoption(props) {
  ...
  useEffect(() => {
    ...
    async function users() {
      await axios
        .get(`http://localhost:1200/username/${props.payid}`)
        .then((result) => {
          setuser1(result.data[0].username);
        });
      await axios
        .get(`http://localhost:1200/username/${props.owedid}`)
        .then((result) => {
          setuser2(result.data[0].username);
        });
    }
  }, [props.owedid, props.payid]);

  function equalcalculate(amt) {
    props.setowedamount(amt / 2);
    props.setowedid();
    return amt / 2;
  }
  ...
  //     setowed(user2amount);
  //     setowedid(v2);
  //   }, [setowed, setowedid, user2amount, v2]);

  //   useEffect(() => {
  //     const total = (amount * +user2percentage) / 100;
  //     setowedid(v2);
  //     setowed(total);
  //   }, [amount, setowed, setowedid, user2percentage, v2]);
      ...
      content = (
        <div className="m-3">
          ...
          <div className="flex justify-between mt-5">
            ...
            {/* <p className="my-auto">${equalcalculate(amount)}.00</p> */}
          </div>
          <div className="flex justify-between mt-5">
            <div className="flex gap-1">
              <img src={owed} alt="" className="rounded-full h-7 w-7 my-auto" />
        ...
        <div className="m-3">
          ...
          <div className="m-5 flex justify-between">
            ...
            <div>
              ...
              <p className="text-xs">
                {/* ₹{amount - (Number(user1amount) + Number(user2amount))}.00 left */}
              </p>
            </div>
          </div>
        </div>
...
export default Updatesplitoption;
